...
Markdown files in `simulations/notes/*.md`, rendered by `marked` and typeset by KaTeX — both vendored under `assets/vendor/`, no CDN. `render.js`'s `protectMath()`/`restoreMath()` shield `$...$`/`$$...$$`/`\(...\)`/`\[...\]` spans from Markdown's backslash-escaping (using U+E000/U+E001 placeholders) before KaTeX sees them.

Neither library is in the shell's `<head>`. `app.js` loads them on demand (`loadAsset`/`ensureMarked`/`ensureKatex`): `marked` only when notes have to be fetched, KaTeX only when the notes panel is first opened — the panel is `max-height: 0` until then, so there's nothing on screen to typeset earlier. A route with no notes, or whose notes are never opened, loads neither. `typesetMath()` guards with a `data-typeset` flag, because running KaTeX twice over the same element re-parses its own output.

The vendored KaTeX CSS keeps only the `woff2` sources (the `woff`/`ttf` fallbacks are stripped, and those files aren't downloaded); browsers fetch only the two or three font faces a given page's maths actually uses. Version query strings on these URLs are set by hand in `app.js` (`KATEX_VERSION`, `MARKED_VERSION`), since `version-assets.js` only rewrites the shell's own `href`/`src` attributes and never sees URLs built in JS — bump them when upgrading a vendored file.
